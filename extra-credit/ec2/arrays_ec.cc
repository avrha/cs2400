...
void fillUP(int numbers[], int count) {
    ...
    ifstream inStream("inputFile1.txt"); //Declares variable inStream and opens flie inputFile1.txt

    //Checks if opening file has failed
    if(inStream.fail())
    {
    ...
    //Runs loop four times
    for(int i = 0; i < count; i++)
    {
        //Inputs the value from inStream to an element in numbers; 
        inStream >> numbers[i];
    }
}
